...
function budgetOverlap(aMin: number | null, aMax: number | null, bMin: number | null, bMax: number | null) {
...
export function scorePeople(a: MatchablePerson, b: MatchablePerson): CompatibilityResult {
  ...
  const languageScore =
    a.languageMatters || b.languageMatters
  ...
  if (sharedLocalities.length) {
    ...
    reasons.push({ factor: 'location', kind: 'DIFFERENCE', score: locationScore, description: 'Different preferred localities' });
  }
  if (budgetScore >= 0.6) {
    reasons.push({ factor: 'budget', kind: 'POSITIVE', score: budgetScore, description: 'Similar budget' });
  } else if (a.minBudget != null && b.minBudget != null) {
    reasons.push({ factor: 'budget', kind: 'DIFFERENCE', score: budgetScore, description: 'Different budget range' });
  }
  if (moveInScore >= 0.6) {
    reasons.push({ factor: 'moveIn', kind: 'POSITIVE', score: moveInScore, description: 'Similar move-in date' });
  } else if (moveGap != null) {
    reasons.push({ factor: 'moveIn', kind: 'DIFFERENCE', score: moveInScore, description: 'Move-in dates are farther apart' });
  }
  if (scaleDistance(a.sleepStart, b.sleepStart, 8) >= 0.7) {
    reasons.push({ factor: 'sleep', kind: 'POSITIVE', score: scaleDistance(a.sleepStart, b.sleepStart, 8), description: 'Similar sleep schedule' });
  } else if (a.sleepStart != null && b.sleepStart != null) {
    reasons.push({ factor: 'sleep', kind: 'DIFFERENCE', score: scaleDistance(a.sleepStart, b.sleepStart, 8), description: 'Different sleep schedules' });
  }
  if (scaleDistance(a.cleanliness, b.cleanliness, 4) >= 0.7) {
    reasons.push({ factor: 'cleanliness', kind: 'POSITIVE', score: scaleDistance(a.cleanliness, b.cleanliness, 4), description: 'Similar cleanliness preference' });
  }
  ...
  if (foodScore >= 0.8) {
    reasons.push({ factor: 'food', kind: 'POSITIVE', score: foodScore, description: 'Compatible food preference' });
  } else if (a.foodPreference && b.foodPreference) {
    reasons.push({ factor: 'food', kind: 'DIFFERENCE', score: foodScore, description: 'Different food preferences' });
  }
  if (sharedLanguages.length) {
    reasons.push({ factor: 'language', kind: 'POSITIVE', score: languageScore, description: `Shared languages · ${sharedLanguages.join(', ')}` });
  } else if ((a.languageMatters || b.languageMatters) && a.languages.length && b.languages.length) {
    reasons.push({ factor: 'language', kind: 'DIFFERENCE', score: languageScore, description: 'No shared language' });
  }
  if (workScore === 1) {
    reasons.push({ factor: 'work', kind: 'POSITIVE', score: workScore, description: 'Similar work schedule' });
  }
...
export function scoreRoom(viewer: MatchablePerson, room: MatchableRoom): CompatibilityResult {
  ...
  if (viewer.localities.includes(room.locality)) {
    reasons.unshift({ factor: 'room-location', kind: 'POSITIVE', score: 1, description: `Room in ${room.locality}` });
  }
