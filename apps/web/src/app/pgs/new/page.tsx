...
import { BedInventoryForm, bedsToPayload, defaultBeds, type BedDraft } from '@/components/pg-bed-inventory';
...
export default function NewPgPage() {
  ...
  const [beds, setBeds] = useState<BedDraft[]>(defaultBeds());
  const backHref = user?.role === 'PG_OWNER' ? '/operator' : '/pgs';
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-8 sm:px-5 sm:py-10">
      <Link href={backHref} className="inline-flex items-center gap-2 text-sm text-muted hover:text-ink">
        <ArrowLeft className="h-4 w-4" /> {user?.role === 'PG_OWNER' ? 'Operator dashboard' : 'PG marketplace'}
      </Link>
      <h1 className="mt-4 text-3xl font-semibold">List a PG</h1>
      <p className="mt-2 text-sm text-muted">Add each bed with room name, label, rent, and availability.</p>
